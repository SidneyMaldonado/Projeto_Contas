...
- `Model/ContasDbContext.cs` — um `DbSet<T>` por entidade. Connection string vem de `appsettings`/config da API. O `OnConfiguring` tem um fallback, usado só quando ninguém configura via DI (`new ContasDbContext()` no `ConexaoTest` e nas ferramentas do EF): `ObterStringConexao()` usa `ConnectionStrings__DefaultConnection` se a variável existir; senão escolhe entre as constantes `ConexaoDesenvolvimento` e `ConexaoPublicacao` por `ASPNETCORE_ENVIRONMENT`/`DOTNET_ENVIRONMENT`; sem ambiente, Debug = desenvolvimento e Release = publicação. As strings repetem as do `appsettings` da Api — mudou o banco, mude nos dois lugares.
...
- OpenAPI via `Microsoft.AspNetCore.OpenApi` (não Swashbuckle) com transformers que injetam o esquema Bearer e marcam `Security` em todo endpoint sem `[AllowAnonymous]`. A UI (`Swashbuckle.AspNetCore.SwaggerUI`) sobe sempre em Development e, nos outros ambientes, só com `Swagger:Habilitado: true`; o endpoint dela é relativo (`../openapi/v1.json`) para funcionar dentro do `PathBase`.
- **Configuração por ambiente** (desde `6af6f90`): `appsettings.json` é a versão de **publicação** (login SQL `user_db_dev`, `PathBase: /fin_back`, Swagger ligado) e `appsettings.Development.json` sobrescreve só o que muda local (`Integrated Security`, `PathBase` vazio). A publicação vai por FTP com o perfil `FTPLabFin_Back` (fora do git) para o IIS em `MS208`, servida em `http://lab.miltecti.com.br/fin_back/`; o perfil tem `EnvironmentName=Production`, gravado no `web.config`. `Integrated Security` não serve no IIS porque a Api roda com a identidade do pool de aplicativos.
- **`PathBase`:** `Program.cs` aplica `UsePathBase` quando a chave vem preenchida. Ele só remove o prefixo quando a requisição chega com ele — no IIS o ASP.NET Core Module já o removeu (sem efeito), atrás de proxy/Docker é ele que tira o `/fin_back`, e local fica vazio.
...
- **Endereço da Api:** `Api:BaseUrl`, com `http://localhost:5210/` no `appsettings.Development.json` e `http://lab.miltecti.com.br/fin_back/` no `appsettings.json` (publicação). A barra final é obrigatória: sem ela o `HttpClient` trata `fin_back` como nome de arquivo e o descarta ao combinar com as rotas relativas.
...
- `Services/ApiConfig.cs` — `BaseUrl` único do app. No Android aponta para `http://lab.miltecti.com.br/fin_back/` em Debug e Release (permite depurar em celular físico contra o servidor); para o emulador contra a Api local, o endereço é `http://10.0.2.2:5210/`, deixado comentado ao lado. Nos demais targets, `http://localhost:5210/`.
- `Platforms/Android/Resources/xml/network_security_config.xml` — o Android 9+ bloqueia HTTP sem TLS, e o servidor ainda não tem HTTPS. A liberação vale só para `lab.miltecti.com.br` e `10.0.2.2`; host HTTP novo precisa entrar nessa lista.
...
- O compose passa `PathBase` (`${API_PATH_BASE:-/fin_back}`) para o container, para a Api responder atrás de um proxy que repasse a subpasta `/fin_back`.
...
- **Credenciais versionadas:** a senha do login `user_db_dev` está em `Contas_Api/appsettings.json` e na constante `ConexaoPublicacao` do `ContasDbContext`, e a `Jwt:Key` também está no `appsettings.json`. Para tirar do repositório, passar por variável de ambiente no IIS (`ConnectionStrings__DefaultConnection`, `Jwt__Key`) — o fallback do `ContasDbContext` já dá precedência a `ConnectionStrings__DefaultConnection`.
- O Swagger está ligado na publicação (`Swagger:Habilitado: true`) por ser um servidor de laboratório; desligar num ambiente de produção de verdade.
