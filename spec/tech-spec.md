...
- `Components/Pages/*.razor` — páginas roteadas (`@page`). `Home.razor` (`/`) e `Login.razor` (`/login`) hoje; páginas de template (`Counter`, `Weather`) ainda presentes mas não fazem parte do domínio.
...
- `Services/AuthHeaderHandler.cs` — `DelegatingHandler` Scoped que injeta `Authorization: Bearer {AuthSession.Token}` em qualquer `HttpClient` tipado registrado com `.AddHttpMessageHandler<AuthHeaderHandler>()`. **Todo novo serviço de API que precise de autenticação deve registrar esse handler** (só `AuthApiService`, usado para login, fica de fora — não tem token ainda nesse ponto).
- Não há camada de "serviço de API" única: cada família de endpoints tem seu próprio `XApiService` tipado (`AuthApiService`, `DashboardApiService`), registrado em `Program.cs` via `AddHttpClient<XApiService>(...)`.
