...
- `Components/Shared/` — `BarraFiltro` (busca + inativos + botão Novo), `AcoesLinha` (Editar/Inativar/Excluir), `Alerta` e `Situacao`. `AcoesLinha` confirma **inline**, sem `confirm()` do JS: um diálogo modal do browser travaria o circuito do Blazor Server.
- `Filtro.cs` / `Formato.cs` (raiz do projeto) — `Filtro.Corresponde(termo, campos...)` compara sem diferenciar maiúsculas nem acentuação (para "divida" achar "dívida"); `Formato` centraliza moeda/data/quantidade em pt-BR.
